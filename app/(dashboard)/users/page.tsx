...
interface AccountsPopupPosition {
  top: number
  left: number
  showAbove: boolean
}
...
export default function UsersPage() {
  ...
  const [accountsPopupPosition, setAccountsPopupPosition] = useState<AccountsPopupPosition | null>(null)
  const accountsButtonRefs = useRef<Map<string, HTMLDivElement>>(new Map())
  ...
  const handleAccountsHover = (userId: string) => {
    const buttonEl = accountsButtonRefs.current.get(userId)
    if (buttonEl) {
      ...
      setAccountsPopupPosition({
        top: showAbove ? rect.top : rect.bottom + 8,
        left: rect.left,
        showAbove
      })
    }
    setHoveredAccounts(userId)
  }

  const handleAccountsLeave = () => {
    setHoveredAccounts(null)
    setAccountsPopupPosition(null)
  }
  ...
  const columns: Column<User>[] = [
    ...
    },
    {
      ...
      render: (user) => (
        <div
          ref={(el) => {
            if (el) accountsButtonRefs.current.set(user._id, el)
          }}
          className="relative"
          onMouseEnter={() => handleAccountsHover(user._id)}
  ...
  const hoveredUser = users.find(u => u._id === hoveredAccounts)
  ...
  return (
    <>
      ...
      {hoveredAccounts && hoveredUser && accountsPopupPosition && hoveredUser.accounts.length > 0 && typeof window !== 'undefined' && createPortal(
        <div
          ...
          style={{
            top: accountsPopupPosition.showAbove ? 'auto' : `${accountsPopupPosition.top}px`,
            bottom: accountsPopupPosition.showAbove ? `${window.innerHeight - accountsPopupPosition.top + 8}px` : 'auto',
            left: `${accountsPopupPosition.left}px`,
          }}
          onMouseEnter={() => setHoveredAccounts(hoveredAccounts)}
          onMouseLeave={handleAccountsLeave}
        >
          <div className="px-4 pb-2 border-b border-gray-100">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Cuentas asociadas</span>
          </div>
          <div className="max-h-48 overflow-y-auto">
            {hoveredUser.accounts.map((account) => (
              <Link
                key={account._id}
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  <i className="ki-duotone ki-briefcase text-gray-400 flex-shrink-0">
                    <span className="path1"></span>
                    <span className="path2"></span>
                  </i>
                  <span className="text-sm font-medium text-gray-900 truncate">{account.name}</span>
                </div>
                <Badge variant={account.role === 'OWNER' ? 'purple' : 'gray'} className="text-[10px] px-1.5 py-0.5 flex-shrink-0 ml-2">
                  {account.role}
                </Badge>
              </Link>
            ))}
